...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		if(FD_ISSET(srvSock,&readfds)){
			...
			sprintf(buf,"*** User '%s' entered from %s/%d. ***\n",user[cliSock-fd2user].name,user[cliSock-fd2user].ip,user[cliSock-fd2user].port);
		...
		for(fd=0;fd<nfds;fd++){
			...
			if( fd!=srvSock && FD_ISSET(fd,&readfds)){
				...
				setenv("PATH",user[fd-fd2user].env,1);


				printf("fd[%d] is active...his name is %s\n",fd,user[fd-fd2user].name);
				fflush(stdout);
				user[fd-fd2user].cur_pos = (user[fd-fd2user].cur_pos+1) % MAX_PIPE_NUM ;
				printf("cur_pos is %d\n",user[fd-fd2user].cur_pos);
				...
				if(decode_msg(fd, strEcho) == EXIT){
					...
					memset(buf,'\0', STR_MAX);
					sprintf(buf,"*** User '%s' left. ***\n",user[fd-fd2user].name);
					
					user[fd-fd2user].available = 0;
					for(i=0;i<MAX_USER;i++){
						if(user[i].available == 1)
							send_msg_to_cli(user[i].fd, buf);					
	...
	for (i=0; i<cmd_list_num; i++) {
		if(i < (cmd_list_num-1)) {
			...
			if(cmd_list[i].pipe_for_others == PIPE_FOR_OTHERS){
				if(pipe_for_others[cliSock-fd2user][0] == -1){
					
					if(pipe(pipe_for_others[cliSock-fd2user]) <0){
						printf("can't create pipe\n");
						return -1;
					}
				}
		...
		if ((chi_pid = fork()) < 0 ) {
			...
			if(i == 0){
				fflush(stdout);
				if(pipe_table[user[cliSock-fd2user].cur_pos] != NULL) {
					close(pipe_table[user[cliSock-fd2user].cur_pos][1]) ;
					dup2(pipe_table[user[cliSock-fd2user].cur_pos][0], 0) ;
					close(pipe_table[user[cliSock-fd2user].cur_pos][0]) ;
				}
			}
			...
			if(i == (cmd_list_num-1)) {
				if(cmd_list[i].pipe_cross_msg == TRUE) {
					...
					close(pipe_for_others[cliSock-fd2user][0]);
					dup2(pipe_for_others[cliSock-fd2user][1],1);
					close(pipe_for_others[cliSock-fd2user][1]);
			...
			if(i == 0){
				if(pipe_table[user[cliSock-fd2user].cur_pos] != NULL) {
					close(pipe_table[user[cliSock-fd2user].cur_pos][1]) ;
					close(pipe_table[user[cliSock-fd2user].cur_pos][0]) ;
					free(pipe_table[user[cliSock-fd2user].cur_pos]) ;
					pipe_table[user[cliSock-fd2user].cur_pos] = NULL ;
				}
...
int decode_msg(int cliSock, char *msg)
{
	...
	for(i=0; i<input_argc; i++)
	{
		if ((input_argv[i][0] == '|') || (input_argv[i][0] == '!')) {
			if(last_str_type == COMMAND) {
				...
				if (input_argv[i][1] != '\0') {
					...
					pipe_extend_round = (user[cliSock-fd2user].cur_pos+pipe_extend_round) % MAX_PIPE_NUM ;
		...
		/*} else if (input_argv[i][0] == '!') {
			...
			if(pipe_for_others[cliSock-fd2user] == NULL){
					pipe_for_others[cliSock-fd2user] = (int *)malloc(2*sizeof(int));
					if(pipe_for_others[cliSock-fd2user]==NULL){
						printf("malloc() error\n");
						exit(1);
					}
					pipe_for_others[cliSock-fd2user][0] = -1;
					pipe_for_others[cliSock-fd2user][1] = -1;
			}
			...
			if(last_str_type == COMMAND) {
				 ...
				 memset(buf,'\0', STR_MAX);
				 sprintf(buf,"*** %s (#%d) just piped '%s' into his/her pipe. ***\n",user[cliSock-fd2user].name,user[cliSock-fd2user].id,whole_command_string);
				 sprintf(user[cliSock-fd2user].pipe_command,"%s",whole_command_string);
				...
				user[cliSock-fd2user].pipe_for_others = PIPE_FOR_OTHERS;
			...
			memset(buf,'\0', STR_MAX);
			sprintf(buf,"*** %s (#%d) just received the pipe from %s (#%d) by '%s' ***\n",user[cliSock-fd2user].name,user[cliSock-fd2user].id,user[pipe_from_user].name,user[pipe_from_user].id,user[pipe_from_user].pipe_command);
			...
			if( last_str_type == EMPTY ){
				if((i+2) < input_argc) {
					...
					sprintf(user[cliSock-fd2user].env,"%s",input_argv[i+2]);
			...
			sprintf(buf,"*** %s told you ***:   %s\n",user[cliSock-fd2user].name,input_argv[i+2]);
			send_msg_to_cli(user[atoi(input_argv[i+1])-1].fd, buf);
			if(cliSock != user[atoi(input_argv[i+1])-1].fd)
				send_msg_to_cli(cliSock, "");
			return 0;
			...
			int index;	
			 memset(buf,'\0', STR_MAX);
			 sprintf(buf,"*** %s yelled ***: %s\n",user[cliSock-fd2user].name,input_argv[i+1]);
			//for(index=0;index <= cur_user;index++)
			//	send_msg_to_cli(user[index].fd, buf);
			...
			int index;
			sprintf(user[cliSock-fd2user].name,"%s",input_argv[i+1]);

			memset(buf,'\0', STR_MAX);
			sprintf(buf,"*** User from %s/%d is named '%s'. ***\n",user[cliSock-fd2user].ip,user[cliSock-fd2user].port,user[cliSock-fd2user].name);
			...
			if( check_cmd(cliSock, input_argv[i]) == FIND ) {
                ...
                send_msg_to_cli(cliSock, buf) ;
				cmd_list[cmd_list_cnt+1].unknown = 1;
				last_str_type = COMMAND ;
				//free(cmd_list);
				//error_flag = 1;
				//continue;
