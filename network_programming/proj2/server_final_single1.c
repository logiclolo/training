...
int main(int argc, char *argv[])
{
	...
	//rc = setsockopt(srvSock, SOL_SOCKET,  SO_REUSEADDR,
	//					(char *)&on, sizeof(on));
	...
	while(1) {
		...
		if(FD_ISSET(srvSock,&readfds)){
			...
			sprintf(user[cur_user].ip,"nctu");
			
			//port
			//user[cur_user].port = ntohs(cliSockaddr.sin_port);
			user[cur_user].port = 5566;
...
int create_one_round_pipe(int cliSock, CMD_STRUCT *cmd_list, int cmd_list_num, int pipe_from_user) 
{
	...
	for (i=0; i<cmd_list_num; i++) {
		printf("i is %d, cmd_list_number is %d\n",i,cmd_list_num);
		fflush(stdout);
		if(i < (cmd_list_num-1)) {
			if (pipe(fd[i]) <0) {
			...
			if(cmd_list[i].pipe_for_others == PIPE_FOR_OTHERS){
				if(pipe_for_others[fd2user(cliSock,user)][0] == -1){
					printf("creating pipe for other\n");	
		...
		if ((chi_pid = fork()) < 0 ) {
			...
			if(i==0){
				if(cmd_list[i].pipe_for_others == PIPE_FROM_USER){
					printf("pipe out!!!!!!!!!\n");
					fflush(stdout);
			...
			if(pipe_from_user >= 0 && cmd_list[i].pipe_for_others == PIPE_FROM_USER){
...
int decode_msg(int cliSock, char *msg)
{
	...
	for(i=0; i<input_argc; i++)
	{
		...
		/*} else if (input_argv[i][0] == '!') {
			...
			cmd_list[cmd_list_cnt].out = PIPE_OUT ;
			cmd_list[cmd_list_cnt].err = NORMAL ;
			cmd_list[cmd_list_cnt].pipe_for_others = PIPE_FROM_USER;
			//user[fd2user(cliSock,user)].pipe_for_others = PIPE_FOR_OTHERS;
			if(input_argv[i][0] == '!') {
			    cmd_list[cmd_list_cnt].err = PIPE_OUT ;
			}
