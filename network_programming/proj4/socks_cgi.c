...
int main(int argc,char *argv[])
{
	...
	int input_argc = 0;
	int num_server = -1;
	int num_server_socks = -1;
	char *batch_file[MAXUSER];
	int  port[MAXUSER];
	int socks_port[MAXUSER];
	char *hostname[MAXUSER];
	char *socks_hostname[MAXUSER];
	int file;
	char *check;
	char cgi_text[] =
	"\
	...
	*/
	...
	strcpy(env,"h1=ep.cs.nthu.edu.tw&p1=7000&f1=t3.txt&sh1=ep.cs.nthu.edu.tw&sp1=8000&h2=&p2=&f2=&sh2=&sp2=&h3=&p3=&f3=&h4=&p4=&f4=&h5=&p5=&f5= HTTP/1.1");
	//printf("%s\n",env);
	//fflush(stdout);
	...
	for(i=0;i<input_argc;i++){
		...
		if(!strncmp(input_argv[i],"sh",2) &&  (input_argv[i][4] != '\0' && input_argv[i][4] != ' ')){
			num_server_socks++;
			socks_hostname[num_server_socks] = input_argv[i]+4;
			printf("%s\n",socks_hostname[num_server_socks]);
		
		}
		if(!strncmp(input_argv[i],"sp",2) &&  (input_argv[i][4] != '\0' && input_argv[i][4] != ' ')){
		
		    socks_port[num_server_socks] = atoi(input_argv[i]+4);
		}


	}
	printf("num_server is %d,num_server_socks is %d\n",num_server,num_server_socks);
	...
	for(i=0;i<conn;i++)
    {
		...
		socks_he = gethostbyname(socks_hostname[i]);
        ...
        client_sin[i].sin_addr = *((struct in_addr *)socks_he->h_addr); 
        client_sin[i].sin_port = htons(socks_port[i]);
		...
		printf("ras address:%s\n",ip);
		fflush(stdout);

		
		input_ip[input_argc_ip] = strtok(ip,".");
		input_ip[input_argc_ip][strlen(input_ip[input_argc_ip])] = '\0';
		
		while(input_ip[++input_argc_ip] = strtok(NULL,"."))
			input_ip[input_argc_ip][strlen(input_ip[input_argc_ip])] = '\0' ;
		...
		package[0] = 4;
		package[1] = 1;
		package[2] = socks_port[i]/256;
		package[3] = socks_port[i]%256;
		for(j=0;j<input_argc_ip;j++)
			package[j+4] = atoi(input_ip[j]);


		for(j=0;j<9;j++){
		      printf("read message[%d]:%u\n",j,package[j]);
		      fflush(stdout);
		}
		...
		write(client_fd[i],package,sizeof(package));
