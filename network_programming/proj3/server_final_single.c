	...
	while(1) {
		...
		for(fd=0;fd<nfds;fd++){
			...
			if( fd!=srvSock && FD_ISSET(fd,&readfds)){
				...
				//}
				//
				if(user[fd2user(fd,user)].ignore==1){
					send_msg_to_cli(fd, "");
					continue;
				}
		...
		if ((chi_pid = fork()) < 0 ) {
			...
			if(i == (cmd_list_num-1)) {
				...
				if(cmd_list[i].pipe_cross_msg == TRUE) {
					...
					//if(cmd_list[i].err == PIPE_OUT) 
					//	dup2(pipe_for_others[fd2user(cliSock,user)][1],2);
					//else
					//	dup2(cliSock, 2) ;
...
int decode_msg(int cliSock, char *msg)
{
	...
	for(i=0; i<input_argc; i++)
	{
		...
		else if (strcmp(input_argv[i], "ignore") == 0) {
			printf("fffffffffkkkkkkk\n");
			user[fd2user(cliSock,user)].ignore = 1;
		...
		else if (strcmp(input_argv[i], "resume") == 0){
				int index;
