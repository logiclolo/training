...
static char bufferParse[BUFSIZE+1];
...
void parse_env(char *msg){
	...
	//1.GET or POST
	if(!strncmp(tmp,"GET",3) || !strncmp(tmp,"get",3))
	...
	setenv("SCRIPT_NAME",queryString , 1) ;
	env = getenv("SCRIPT_NAME");
	printf("SCRIPT_NAME:::\n%s\n",env);


	
	//3.QUERY_STRING starts from '?'
	begin = tmp + strcspn(tmp,"?");
	begin++;
	...
	begin = end;
	begin++;//skip '\n'
	begin = begin + 6;//skip Host:
	
	end = begin + strcspn(begin,":");
	queryString = mySavestring(begin,end-begin);
	setenv("REMOTE_HOST",queryString , 1) ;
...
void handle_socket(int fd)
{
    ...
    if (!strncmp(&buffer[0],"GET / HTTP\0",10)||!strncmp(&buffer[0],"get / http\0",10) ){
		...
		parse_env(buffer);
		status = execvp(getenv("SCRIPT_NAME",NULL));		
