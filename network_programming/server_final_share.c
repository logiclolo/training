	...
	for (i=0; i<cmd_list_num; i++) {
		if(i < (cmd_list_num-1)) {
			...
			if(cmd_list[i].pipe_for_others == PIPE_FOR_OTHERS){
				/*
				//if(pipe_for_others[cur_user][0] == -1){
				if(pipe_for_others[2*cur_user] == -1){
					
					//if(pipe(pipe_for_others[cur_user]) <0){
					if(pipe(&pipe_for_others[2*cur_user]) <0){
						printf("can't create pipe\n");
						return -1;
					}
				}
